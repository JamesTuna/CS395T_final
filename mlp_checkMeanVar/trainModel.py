...
# optimizer related
parser.add_argument('--lamb', type=float, default=1.0, help='weight between mean and std in mean-var optimization')
...
trainer = RobustTrainer(model, train_loader=train_loader, test_loader=test_loader,loss = loss,
                noise_scale=args.noise, n_noises=None,train_epochs=args.epoch,lr=args.lr,
...
trainer.mean_var_optimization(lamb=args.lamb, print_step=args.ps, optimizer=args.opt,
                              reduce_lr_per_epochs=args.lr_decay_epoch, reduce_rate=args.lr_decay_rate,
                              logdir=args.logdir, saveas=args.save_as, save_per_epochs=args.save_per_epochs)
