...
def regressMLP(model,X,noise):
    ...
    for i in range(n_layers):
        ...
        if layer != 1:
            ...
            varA_m1 = varA.mean()/(meanA**2).mean() #run1
            varA_m2 = (varA/(meanA+1e-10)**2).mean() #run2

            print("var est1: ",varA_m1)
            print("var est2: ",varA_m2)
        else:
            meanA = X
            varA_m1 = 0
            varA_m2 = 0
        ...
        sol = rbSolve(meanA,Y,varA_m1,varx=noise**2)
        newW = sol[:-1,:]
        newb = sol[-1,:]
        newModel.Ws[i] = newW
        newModel.bs[i] = newb
...
if __name__ == "__main__":
    ...
    newModel = regressMLP(model,A_train[:10000],noise)
